...
<style>
    ...
    @media (max-width: 768px) {
        ...
        .stTabs [data-baseweb="tab"] { font-size: 0.6rem !important; padding: 4px 0 3px 0 !important; }
    }

    .stTabs [role="tab"] { min-width: 0 !important; }
...
def compute_india_mf_aggregate() -> dict:
    ...
    for mf_entry in MF_CONFIG:
        ...
        try:
            ...
            # Use Live Value Always
            if not hist.empty:
                latest_nav = float(hist["Close"].iloc[-1])
                candidate_value = latest_nav * units
                value_inr = candidate_value if candidate_value > 0 else file_value_inr
            else:
...
<div class="kpi-label">{bottom_label}</div>
    ...
    st.markdown('''<div style="font-family: 'Space Grotesk', system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color:#16233a; font-size:0.75rem; margin:10px 0 4px 0;">Today's Gains</div>''', unsafe_allow_html=True)
    ...
    if agg_for_heatmap is None or agg_for_heatmap.empty:
        ...
        if mf_val_inr > 0 or mf_day_pl_inr != 0.0:
            ind_mf_row = {
                ...
                "Units": 0.0, "PriceUSD": 0.0, "ValueAED": mf_val_inr / AED_TO_INR if mf_val_inr > 0 else 0.0,
                "PurchaseAED": 0.0, "DayPct": 0.0, "DayPLAED": mf_day_pl_inr / AED_TO_INR,
                "DayPLINR": mf_day_pl_inr, "TotalPct": 0.0, "TotalPLAED": 0.0, "WeightPct": 0.0,
            }
        ...
        fig = px.treemap(hm, path=["Name"], values="SizeForHeatmap", color="DayPLINR", color_continuous_scale=[COLOR_DANGER, "#16233a", COLOR_SUCCESS], color_continuous_midpoint=0, custom_data=["DayPLINR", "Ticker", "DayPLKLabel"])
        fig.update_traces(hovertemplate="<b>%{label}</b><br>Ticker: %{customdata[1]}<br>Day P&L: ₹%{customdata[0]:,.0f}<extra></extra>", texttemplate="%{label}<br>%{customdata[2]}", textfont=dict(family="Space Grotesk, sans-serif", color="#e6eaf0", size=11), marker=dict(line=dict(width=0)), root_color=COLOR_BG)
        fig.update_layout(margin=dict(t=0, l=0, r=0, b=0), paper_bgcolor=COLOR_BG, plot_bgcolor=COLOR_BG, coloraxis_showscale=False, font=dict(family="Space Grotesk, sans-serif"))
...
with sv_tab:
    ...
    if sv_positions.empty:
        ...
        with c1:
            st.markdown(f"""<div class="card mf-card"><div class="kpi-label">TODAY'S PROFIT</div><div class="kpi-mid-row"><div class="kpi-number">AED {sv_day_pl_aed:,.0f}</div><div class="kpi-number">{(sv_day_pl_aed/(sv_total_val_aed-sv_day_pl_aed)*100 if (sv_total_val_aed-sv_day_pl_aed)>0 else 0):+.2f}%</div></div><div class="kpi-label">US STOCKS</div></div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="card mf-card"><div class="kpi-label">TOTAL PROFIT</div><div class="kpi-mid-row"><div class="kpi-number">AED {sv_total_pl_aed:,.0f}</div><div class="kpi-number">{sv_total_pl_pct:+.2f}%</div></div><div class="kpi-label">US STOCKS</div></div>""", unsafe_allow_html=True)
        with c3:
            st.markdown(f"""<div class="card mf-card"><div class="kpi-label">TOTAL HOLDING</div><div class="kpi-mid-row"><div class="kpi-number">AED {sv_total_val_aed:,.0f}</div><div class="kpi-number">{fmt_inr_lacs_from_aed(sv_total_val_aed, AED_TO_INR)}</div></div><div class="kpi-label">US STOCKS</div></div>""", unsafe_allow_html=True)
        ...
        fig_sv = px.treemap(hm_sv, path=["Name"], values="SizeForHeatmap", color="DayPLAED", color_continuous_scale=[COLOR_DANGER, "#16233a", COLOR_SUCCESS], color_continuous_midpoint=0, custom_data=["DayPLAED", "Ticker", "DayPLLabel"])
...
<div class="kpi-label">COST: AED {us_total_purchase_aed:,.0f}</div>
<div class="kpi-label">PROFIT: AED {us_total_pl_aed:,.0f}</div>
</div>
<div class="kpi-mid-row">
<div class="kpi-number">AED {us_total_val_aed:,.0f}</div>
<div class="kpi-number">{us_total_pl_pct:+.2f}%</div>
</div>
<div class="kpi-label">PORTFOLIO AGGREGATE</div>
        ...
        for row in sorted_pos:
            ...
            val_aed_str = f"AED {row['ValueAED']:,.0f}"
            pl_aed_str = f"{'+ ' if row['TotalPLAED'] >= 0 else ''}AED {row['TotalPLAED']:,.0f}"
            pl_pct_str = f"{row['TotalPct']:+.2f}%"
            
            st.markdown(
f"""<div class="card mf-card">
<div class="kpi-top-row">
<div class="kpi-label" style="font-weight:600;">{units_str}</div>
<div class="kpi-label" style="font-weight:600; color:{COLOR_SUCCESS if row['TotalPct'] >=0 else COLOR_DANGER} !important;">{pl_pct_str}</div>
</div>
<div class="kpi-mid-row">
...
<div class="kpi-label" style="color:{COLOR_SUCCESS if row['TotalPLAED'] >=0 else COLOR_DANGER} !important;">{pl_aed_str}</div>
</div>
</div>""",
                unsafe_allow_html=True,
            )
...
with mf_tab:
    if not MF_CONFIG:
        ...
        for row in mf_rows:
            display_name = row["scheme"].replace(" Fund Growth", "")
